...
@Deleter(EawolfeSantiadmTevinl1Link)
@Getter(EawolfeSantiadmTevinl1Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(EawolfeSantiadmTevinl1Link)
        print "Found ", len(ls), " links."
        count = 0
        for l in ls:
            if not is_valid(l.full_url):
                self.frame.delete(EawolfeSantiadmTevinl1Link, l)
                count += 1
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_EawolfeSantiadmTevinl1".format(CrawlerFrame.app_id),
